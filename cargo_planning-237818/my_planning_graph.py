...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        #selecionando cada action
        for ac in self.all_actions:
            #Criando Node para acao  
            ac_node = PgNode_a(ac)
            if ac_node.prenodes.issubset(self.s_levels[level]):
                #Adicionando acao na lista de nodes de action
                action_nodes.append(ac_node)
                for node_state in self.s_levels[level]:
                    node_state.children.add(ac_node)
                    ac_node.parents.add(node_state)
    ...
    def add_literal_level(self, level):
        ...
        state_nodes = set()
        for ac_node in self.a_levels[level-1]:
            #Recuperando nodes anteriores a acao
            for state_n in ac_node.effnodes:
                #Criando referencia entre nodes
                state_nodes.add(state_n)
                state_n.parents.add(ac_node)
                ac_node.children.add(state_n)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #Verificando efeitos e ajustando sinais
